...
echo $PATH | grep $PS3DEV/host/ppu/bin 1> /dev/null || { echo "ERROR: Add $PS3DEV/host/ppu/bin to your path before continuing."; exit 1; }

## Check for $PS3DEV/spu/bin in the path.
echo $PATH | grep $PS3DEV/host/spu/bin 1> /dev/null || { echo "ERROR: Add $PS3DEV/host/spu/bin to your path before continuing."; exit 1; }
